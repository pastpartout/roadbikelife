...
//    }
  ...
  {
    $contentItemActivityIds = $this->ContentItemActivityIds();
    if ($contentItemActivityIds) {
      $stravaActivities = $this->stravaApi->get('athlete/activities', ['per_page' => 30]);
      $this->stravaActivities = array_filter($stravaActivities, function ($activity) use ($contentItemActivityIds) {
        return in_array($activity->id, $contentItemActivityIds);
      });
    } else {
  ...
  private function ContentItemActivityIds()
  {
    ...
    if (!empty($contentId)) {
      ...
      $query->where($db->quoteName('field_id') . ' = 5');
      $query->where($db->quoteName('item_id') . ' = ' . $db->quote($contentId));

      $db->setQuery($query);

      $ids = $db->loadAssocList('value');
