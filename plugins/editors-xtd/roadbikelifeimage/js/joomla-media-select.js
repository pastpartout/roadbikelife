...
/**
 * Event Listener that updates the Joomla.selectedMediaFileRbl
 * to the selected file in the media manager
 */


document.addEventListener('onMediaFileSelected', async e => {
  Joomla.selectedMediaFileRbl = e.detail;
  ...
  const optionsEl = container.querySelector('joomla-field-mediamore-rbl');
  ...
  if (Joomla.selectedMediaFileRbl.path) {
    let type;

    if (images.includes(Joomla.selectedMediaFileRbl.extension.toLowerCase())) {
      type = 'images';
    } else if (audios.includes(Joomla.selectedMediaFileRbl.extension.toLowerCase())) {
      type = 'audios';
    } else if (videos.includes(Joomla.selectedMediaFileRbl.extension.toLowerCase())) {
      type = 'videos';
    } else if (documents.includes(Joomla.selectedMediaFileRbl.extension.toLowerCase())) {
      type = 'documents';
    }

    if (type) {
      container.insertAdjacentHTML('afterbegin', `<joomla-field-mediamore-rbl
...
const getImageSize = url => new Promise((resolve, reject) => {
  ...
  img.onload = () => {
    Joomla.selectedMediaFileRbl.width = img.width;
    Joomla.selectedMediaFileRbl.height = img.height;
    resolve(true);
  };
...
const insertAsImage = async (media, editor, fieldClass) => {
  if (media.url) {
    ...
    if (parts.length > 1) {
      ...
      Joomla.selectedMediaFileRbl.url = parts[1];

      if (media.thumb_path) {
        Joomla.selectedMediaFileRbl.thumb = media.thumb_path;
      } else {
        Joomla.selectedMediaFileRbl.thumb = false;
      }
    } else if (media.thumb_path) {
      Joomla.selectedMediaFileRbl.url = media.url;
      Joomla.selectedMediaFileRbl.thumb = media.thumb_path;
    }
  } else {
    Joomla.selectedMediaFileRbl.url = false;
  }

  if (Joomla.selectedMediaFileRbl.url) {
    let attribs;
    let isLazy = '';
    let alt = '';
    let appendAlt = '';
    ...
    if (!isElement(editor)) {
      ...
      attribs = currentModal.querySelector('joomla-field-mediamore-rbl');
      ...
      if (attribs) {
        ...
        if (attribs.getAttribute('is-lazy') === 'true') {
          isLazy = ` loading="lazy" width="${Joomla.selectedMediaFileRbl.width}" height="${Joomla.selectedMediaFileRbl.height}"`;

          if (Joomla.selectedMediaFileRbl.width === 0 || Joomla.selectedMediaFileRbl.height === 0) {
            try {
              await getImageSize(Joomla.selectedMediaFileRbl.url);
              isLazy = ` loading="lazy" width="${Joomla.selectedMediaFileRbl.width}" height="${Joomla.selectedMediaFileRbl.height}"`;
            } catch (err) {
              isLazy = '';
            }
      ...
      if (figCaption) {
        imageElement = `<figure${figClasses}><img src="${Joomla.selectedMediaFileRbl.url}"${classes}${isLazy}${alt} data-path="${Joomla.selectedMediaFileRbl.path}"/><figcaption>${figCaption}</figcaption></figure>`;
      } else {
        imageElement = `{contentimage ${Joomla.selectedMediaFileRbl.url}|${alt}}`;
      }
      ...
      if (Joomla.selectedMediaFileRbl.width === 0 || Joomla.selectedMediaFileRbl.height === 0) {
        try {
          await getImageSize(Joomla.selectedMediaFileRbl.url); // eslint-disable-next-line no-empty
        } catch (err) {
          Joomla.selectedMediaFileRbl.height = 0;
          Joomla.selectedMediaFileRbl.width = 0;
        }
      }

      editor.value = `${Joomla.selectedMediaFileRbl.url}#joomlaImage://${media.path.replace(':', '')}?width=${Joomla.selectedMediaFileRbl.width}&height=${Joomla.selectedMediaFileRbl.height}`;
    ...
    if (!isElement(editor)) {
      ...
      if (attribs) {
        ...
        if (embedable && embedable === 'true') {
          if (type === 'audios') {
            outputText = `<audio controls src="${Joomla.selectedMediaFileRbl.url}"></audio>`;
          }

          if (type === 'documents') {
            // @todo use ${Joomla.selectedMediaFileRbl.filetype} in type
            const title = attribs.getAttribute('title');
            outputText = `<object type="application/${Joomla.selectedMediaFileRbl.extension}" data="${Joomla.selectedMediaFileRbl.url}" ${title ? `title="${title}"` : ''} width="${attribs.getAttribute('width')}" height="${attribs.getAttribute('height')}">
  ${Joomla.Text._('JFIELD_MEDIA_UNSUPPORTED').replace('{tag}', `<a download href="${Joomla.selectedMediaFileRbl.url}">`).replace(/{extension}/g, Joomla.selectedMediaFileRbl.extension)}
</object>`;
          }

          if (type === 'videos') {
            outputText = `<video controls width="${attribs.getAttribute('width')}" height="${attribs.getAttribute('height')}">
  <source src="${Joomla.selectedMediaFileRbl.url}" type="${Joomla.selectedMediaFileRbl.fileType}">
</video>`;
          }
        } else if (Joomla.editors.instances[editor].getSelection() !== '') {
          outputText = `<a download href="${Joomla.selectedMediaFileRbl.url}">${Joomla.editors.instances[editor].getSelection()}</a>`;
        } else {
          const name = /([\w-]+)\./.exec(Joomla.selectedMediaFileRbl.url);
          outputText = `<a download href="${Joomla.selectedMediaFileRbl.url}">${Joomla.Text._('JFIELD_MEDIA_DOWNLOAD_FILE').replace('{file}', name[1])}</a>`;
        }
      }
      ...
      editor.value = Joomla.selectedMediaFileRbl.url;
...
const execTransform = async (resp, editor, fieldClass) => {
  if (resp.success === true) {
    ...
    if (Joomla.selectedMediaFileRbl.extension && images.includes(media.extension.toLowerCase())) {
      return insertAsImage(media, editor, fieldClass);
    }

    if (Joomla.selectedMediaFileRbl.extension && audios.includes(media.extension.toLowerCase())) {
      return insertAsOther(media, editor, fieldClass, 'audios');
    }

    if (Joomla.selectedMediaFileRbl.extension && documents.includes(media.extension.toLowerCase())) {
      return insertAsOther(media, editor, fieldClass, 'documents');
    }

    if (Joomla.selectedMediaFileRbl.extension && videos.includes(media.extension.toLowerCase())) {
...
customElements.define('joomla-field-mediamore-rbl', JoomlaFieldMediaOptions);
